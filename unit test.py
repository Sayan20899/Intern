...
class FactoryGUI:
    ...
    def schedule_jobs(self):
        ...
        current_time = 0
        job_completion = {}
        job_end_times = {}  # To track the end time of each job
            ...
            for job in self.batch_jobs:
                ...
                if job_id not in job_completion and arrival_time <= self.tcount:
                    if job_id in job_end_times:
                        current_time = job_end_times[job_id]  # Get the end time of the last job with the same job ID

                    self.process_job(job_id, family, current_time, processing_time)
            ...
            for cell_id in manufacturing_process[family]:
                ...
                if cell:
                    ...
                    if chosen_resource:
                        ...
                        start_time = 0  # Ensure start time is after current time and tcount

                        # Ensure sequential processing
                        if any(task[0] == job_id for task in self.schedule):
                            previous_end_time = max(task[5] for task in self.schedule if task[0] == job_id)
                            start_time = max(start_time, previous_end_time)
                            previous_task = next((task for task in self.schedule if task[3] == chosen_resource["rid"]),
                                                 None)
                            if previous_task:
                                previous_job_end_time = previous_task[5]
                                previous_family = previous_task[1]
                        ...
                        if not any(task[3] == chosen_resource["rid"] for task in self.schedule):
                            ...
                            next_job_same_family = any(task[1] == family and task[3] == chosen_resource["rid"] for task in self.schedule)
                            if not next_job_same_family:
                                previous_job_end_time = max(task[5] for task in self.schedule if task[3] == chosen_resource["rid"])
                                start_time = previous_job_end_time + setup_time

                            elif next_job_same_family != family:
                                start_time = max(task[5] for task in self.schedule if task[3] == chosen_resource["rid"]) + setup_time
                            else:
                                previous_job_end_time = max(
                                    task[5] for task in self.schedule if task[3] == chosen_resource["rid"])
                                start_time = previous_job_end_time
    ...
    def draw_gantt_chart(self):
        ...
        for job in self.batch_jobs:
            ...
            if job_id not in job_colors:
                job_colors[job_id] = generate_color()  # Assign a unique color to each job ID
