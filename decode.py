            ...
            for cell_id in manufacturing_process[family]:
                ...
                if cell:
                    ...
                    if chosen_resource:
                        ...
                        # Update current time considering setup time for the next job
                        #current_time += chosen_resource["setup_time"][int(family) - 1]

                else:
                    break  # Exit the loop if the cell is not found
        else:
            print(
                f"Not all required families found in manufacturing process. Unable to process job {job_id}.")  # Debug print
